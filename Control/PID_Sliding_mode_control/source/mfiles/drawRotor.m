...
rod2 = [output(1), output(2)-diameter, output(3)]';
rod3 = [output(1)-diameter*sqrt(3)/2, output(2)-diameter/2, output(3)]';
rod4 = [output(1)-diameter*sqrt(3)/2, output(2)+diameter/2, output(3)]';
rod5 = [output(1), output(2)+diameter, output(3)]';
rod6 = [output(1)+diameter*sqrt(3)/2, output(2)+diameter/2, output(3)]';
rod7 = [output(1)+diameter*sqrt(3)/2, output(2)-diameter/2, output(3)]';
...
drawLine(body, R*(rod6 - body) + body, [1 .4 .4]);
drawLine(body, R*(rod7 - body) + body, [1 .4 .4]);
...
elseif nargin == 3
	if ischar(color) && length(color) > 1
		line([X(1), Y(1)], [X(2), Y(2)], [X(3), Y(3)], 'LineWidth', 2, 'Color', color(1), 'LineStyle', color(2:end));
	else
		line([X(1), Y(1)], [X(2), Y(2)], [X(3), Y(3)], 'LineWidth', 2, 'Color', color);
	end
else
	if ischar(color) && length(color) > 1
		line([X(1), Y(1)], [X(2), Y(2)], [X(3), Y(3)], 'LineWidth', width, 'Color', color(1), 'LineStyle', color(2:end));
	else
