...
for i = 1:size(dx, 2)
	...
	if any(yz(:,i)) == 0
		yz(:,i) = cross(dx(:,i), [0 0 -1]+0.1*rand(1)*[1 0 0]);
	end
	yz(:,i) = yz(:,i)/norm(yz(:,i));
end
...
repulsive = 20*yz*diag(max(min(-derivative(sqrt(sum(dx.^2)), [obj_name '_dx']) - threshold, sat(2)), sat(1)));
% repulsive = 20*yz.*(evaluate(sat, -derivative(sqrt(sum(dx.^2)), [obj_name '_dx']) - threshold));
repulsive = sum(repulsive, 2);
